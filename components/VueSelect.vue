<template>
    <div class="vue-select" v-click-outside="hide">
        <div :class="'vue-select__current'" @click="suggest = !suggest">
            {{ comCurrent }}
...
export default {
    props: ['option', 'current', 'placeholder', 'success'],
    data() {
        return {
            suggest: false,
    ...
    computed: {
        comCurrent2() {
            return this.current != null && this.option.length > 0
                ? this.option.find(x => x.id === Number.parseInt(this.current)).name
                : ''
        },
        comCurrent() {
            if(this.current != null && this.option.length > 0) {
                let filter = this.option.find(x => x.id === Number.parseInt(this.current))
                if(filter) 
                    return filter.name
            }
